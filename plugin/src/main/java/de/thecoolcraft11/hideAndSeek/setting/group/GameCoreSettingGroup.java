...
public final class GameCoreSettingGroup implements SettingGroup {
    ...
    public List<SettingSpec> settings() {
        return List.of(
                ...
                new IntegerSettingSpec("game.hiding-time", 60, 10, 600, "Hiding phase duration in seconds", Material.CLOCK),
                new IntegerSettingSpec("game.seeking-time", 300, 60, 1800, "Seeking phase duration in seconds", Material.CLOCK),
                new BooleanSettingSpec("game.hider-invisibility", false, "Grant hiders invisibility during hiding phase", Material.POTION, true),
                new BooleanSettingSpec("game.world-border.damage-hiders-outside", true, "Damage hiders when they go outside the world border", Material.BARRIER, true),
                ...
                new BooleanSettingSpec("game.maps.use-map-timings", true, "Use hiding/seeking times from map config if available, otherwise use global settings", Material.CLOCK, true),
                new BooleanSettingSpec("game.maps.use-map-seeker-count", true, "Use seeker configuration from map config if available, otherwise use global settings", Material.IRON_SWORD, true),
                new BooleanSettingSpec("game.maps.use-map-player-limits", true, "Use player count recommendations from map config if available", Material.PLAYER_HEAD, true),
                new BooleanSettingSpec("game.maps.use-map-setting-overrides", true, "Apply map setting-overrides from maps.yml during a round", Material.COMMAND_BLOCK, true),
                new BooleanSettingSpec("game.maps.show-round-start-map-info-title", true, "Show a map info title when the HIDING phase starts", Material.NAME_TAG, true),
                new EnumSettingSpec<>(
                        "game.maps.round-start-map-info-display-mode",
                        MapInfoDisplayMode.class,
                        MapInfoDisplayMode.NAME_AUTHOR_DESCRIPTION,
                        "Which map info fields should be shown in the HIDING start title",
                ...
                new IntegerSettingSpec(
                        ...
                        1,
                        10,
                        "Number of seekers (if random distribution is enabled)",
                        Material.IRON_SWORD,
                        (plugin, value) -> {
                ...
                new IntegerSettingSpec(
                        ...
                        1,
                        20,
                        "Health of hiders (in half-hearts)",
                ...
                new FloatSettingSpec("game.block-form.view-height", 0.1f, 0f, 1.5f, "View Height of player when they hide in a block", Material.LADDER)
        );
    }
}
