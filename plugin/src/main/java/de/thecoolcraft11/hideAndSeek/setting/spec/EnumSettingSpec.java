...
public final class EnumSettingSpec<E extends Enum<E>> implements SettingSpec {
    ...
    public void register(HideAndSeek plugin, SettingValueResolver resolver, SettingIconHelper iconHelper) {
        plugin.getConfigRegistry().register("settings." + key, enumClass, fallback);
        plugin.getSettingRegistry().register(SettingDefinition.builder(key, SettingType.ENUM, enumClass).defaultValue(
                resolver.getEnum(plugin, key, enumClass, fallback)).description(description).customIcon(
                icon).valueIcons(valueIcons).build());
    }
}
